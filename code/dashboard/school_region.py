...
school2020 = pd.ExcelFile(current_dir+'/component/data/학교,학과별 데이터셋_전처리/2020년_상반기_시도별.xlsx')
school2021 = pd.ExcelFile(current_dir+'/component/data/학교,학과별 데이터셋_전처리/2020년_상반기_시도별.xlsx')
school2022 = pd.ExcelFile(current_dir+'/component/data/학교,학과별 데이터셋_전처리/2020년_상반기_시도별.xlsx')
school2023 = pd.ExcelFile(current_dir+'/component/data/학교,학과별 데이터셋_전처리/2020년_상반기_시도별.xlsx')
...
with tab1:
    ...
    st.write('세부 지역별 사고 수 현황 분석을 위해 시도별 관할 교육(지원)청을 기준으로 분류하여 각 연도마다 제공합니다. 각 시도 내의 세부 지역별 사고 발생 상황을 더욱 구체적으로 파악할 수 있습니다')
    st.markdown('######')
